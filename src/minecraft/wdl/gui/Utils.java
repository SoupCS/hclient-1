...
class Utils {
	...
	/**
	 * Draws a dark background, similar to {@link GuiScreen#drawBackground} but darker.
	...
	public static void drawDarkBackground(int top, int left, int bottom, int right) {
		...
		DrawHelper.drawRect(left, top, right, bottom, HClient.getClientColorBackground().getRGB());
	}
...
class TextEntry implements IGuiListEntry {
	...
	@Override
	public void func_192634_a(int slotIndex, int x, int y, int listWidth, int slotHeight,
...
class LinkEntry extends TextEntry {
	...
	public void func_192634_a(int slotIndex, int x, int y, int listWidth,
			int slotHeight, int mouseX, int mouseY, boolean isSelected, float t) {
		if (y < 0) {
			return;
		}
		
		super.func_192634_a(slotIndex, x, y, listWidth, slotHeight, mouseX,
				mouseY, isSelected, t);
