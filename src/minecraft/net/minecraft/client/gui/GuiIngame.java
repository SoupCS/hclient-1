...
import neko.itskekoffcode.clientapi.event.event.Event2D;
...
public class GuiIngame extends Gui
{
    ...
    public String playercord;
    public String bots;
    public String memory;
    public String video;
    public String cpu;
    public String fpsandping;
    public String cords;
    public String date;
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Event2D event = new Event2D(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight());
        ...
        if (mc.gameSettings.showDebugInfo) {
            ...
            Minecraft.getMinecraft().neverlose900_15.drawStringWithShadow("HCLIENT", 7.5, 10, new Color(22, 97, 141).getRGB());
            Minecraft.getMinecraft().neverlose900_15.drawStringWithShadow("HCLIENT", 8, 10.5, -1);
            Minecraft.getMinecraft().neverlose500_15.drawStringWithShadow("| " + text, 7 + Minecraft.getMinecraft().neverlose500_15.getStringWidth("HCLIENT | "), 10.5, -1);
            ...
            for (int l = 0; l <= 90; l = l + 1) {
                DrawHelper.drawSmoothRect(l + 6, 184, l + 5, 185, DrawHelper.getClientColor(5, l, 2).getRGB());
            }
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("hclient.dev", 28, 190, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("user", 9, 200, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("[" + mc.player.getName() + "]", 93 - Minecraft.getMinecraft().neverlose.getStringWidth("[" + mc.player.getName() + "]"), 200, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("server", 9, 208, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("[" + server + "]", 93 - Minecraft.getMinecraft().neverlose.getStringWidth("[" + server + "]"), 208, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("time", 9, 216, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("[" + time + "]", 93 - Minecraft.getMinecraft().neverlose.getStringWidth("[" + time + "]"), 216, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("latency", 9, 224, -1);
            Minecraft.getMinecraft().neverlose.drawStringWithShadow("[" + ping + "]", 93 - Minecraft.getMinecraft().neverlose.getStringWidth("[" + ping + "]"), 224, -1);
            /*String server = mc.isSingleplayer() ? "local" : mc.getCurrentServerData().serverIP.toLowerCase();
            String time = (new SimpleDateFormat("HH:mm:ss")).format(Calendar.getInstance().getTime());
            String bots = String.valueOf(Bot.bots.size());
            String text = "HClient | " + mc.player.getName() + " | " + server + " | " + mc.getDebugFPS() + " fps" + " | " + time + " | Total bots: " + bots;
            DrawHelper.drawSmoothRect(5, 6, Minecraft.getMinecraft().neverlose.getStringWidth(text) + 9, 18, new Color(31, 31, 31, 255).getRGB());
            for (float l = 0; l < Minecraft.getMinecraft().neverlose.getStringWidth(text) + 4; l += 1f) {
                DrawHelper.drawSmoothRect(5 + l, 5, l + 6, 6, DrawHelper.getClientColor(5, l, 3).getRGB());
            }
            DrawHelper.drawSmoothRect(5, 6f, Minecraft.getMinecraft().neverlose.getStringWidth(text) + 9, 6.5f, new Color(20, 20, 20, 100).getRGB());

             */
            //Minecraft.getMinecraft().neverlose.drawStringWithShadow(text, 7, 10.5, -1);
            initHUDComponents(scaledresolution);
            if (!(mc.currentScreen instanceof GuiChat)) {
                String fpsandping = "FPS: " + Minecraft.getDebugFPS() + " Ping: " + Objects.requireNonNull(mc.getConnection()).getPlayerInfo(mc.player.getUniqueID()).getResponseTime();
    ...
    private void initHUDComponents(ScaledResolution sr) {
        // variables
        ...
        long l = j - k;
        String time = (new SimpleDateFormat("HH:mm")).format(Calendar.getInstance().getTime());
        String key = "Key: HTest; Till: Never gon..";
        String server = mc.isSingleplayer() ? "local" : mc.getCurrentServerData() != null ? mc.getCurrentServerData().serverIP.toLowerCase() : "null";
        String proxy = "Total Proxy: " + ProxyParser.proxies.size();
        String watermark = "HClient | " + mc.player.getName() + " | " + server;
        String coords = String.format("Yaw/Pitch: %s/%s", String.valueOf(mc.player.rotationYaw).split("\\.")[0], String.valueOf(mc.player.rotationPitch).split("\\.")[0]);
        assert this.mc.getRenderViewEntity() != null;
        String playercord = String.format("XYZ: %.3f / %.5f / %.3f", this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().getEntityBoundingBox().minY, this.mc.getRenderViewEntity().posZ);
        String bots = String.format("Total Bots: %d", Bot.bots.size());
        String memory = String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i));
        String cpu = String.format("CPU: %s", OpenGlHelper.getCpu());
        String video = String.format("Video: %s", GlStateManager.glGetString(7937));
        String core = mc.isSingleplayer() ? "Core: localhost (Singleplayer?)" : "Core: " + mc.player.getServerBrand();
        String fpsandping = "FPS: " + Minecraft.getDebugFPS() + " Ping: " + Objects.requireNonNull(mc.getConnection()).getPlayerInfo(mc.player.getUniqueID()).getResponseTime();
        String cords = "X:" + (int) mc.player.posX + " Y:" + (int) mc.player.posY + " Z:" + (int) mc.player.posZ;
        String date = (new SimpleDateFormat("dd/MM/yyyy")).format(Calendar.getInstance().getTime());
        if (core.contains("git:")) { core = "Core: " + mc.player.getServerBrand().split("\\(")[0].replace(" ", "") + mc.player.getServerBrand().split("\\)")[1];}
        ...
        //list.add(video);
        //list.add(time);
        drawHUDBackground(watermark, "0", "0", "0", core, list);
        drawHUD(list, fpsandping, cords, time, date);
        //DrawHelper.drawImage(new ResourceLocation("hclient/test.jpg"), 4, 120, 150, 150);
    }
    ...
    private void drawHUDBackground(String info, String xyz, String cpu, String key, String core, List<String> list) {
        int length = mc.fontRendererObj.getStringWidth(info);
        String builder = String.valueOf(list.size() + 1) +
                0;
        if (mc.fontRendererObj.getStringWidth(xyz) > mc.fontRendererObj.getStringWidth(info)) { length = mc.fontRendererObj.getStringWidth(xyz); }
        if (mc.fontRendererObj.getStringWidth(cpu) > mc.fontRendererObj.getStringWidth(xyz) && mc.fontRendererObj.getStringWidth(info) < mc.fontRendererObj.getStringWidth(cpu)) { length = mc.fontRendererObj.getStringWidth(cpu); }
        if (mc.fontRendererObj.getStringWidth(key) > length) { length = mc.fontRendererObj.getStringWidth(key); }
        if (mc.fontRendererObj.getStringWidth(core) > mc.fontRendererObj.getStringWidth(info) && mc.fontRendererObj.getStringWidth(watermark) < mc.fontRendererObj.getStringWidth(core)) { length = mc.fontRendererObj.getStringWidth(core); }
        DrawHelper.drawGlowRoundedRect(4, 4, length + 32, Integer.parseInt(builder), new Color(10, 10, 10, 223).getRGB(), 5, 5);
    }
    private void drawHUD(List<String> list, String fpsandping, String cords, String time, String date) {
        ...
        for (String s : list) {
            if (!Strings.isNullOrEmpty(s)) {
                mc.sfui16.drawString(s, 9F, start, Color.white.getRGB());
                start = start + 10;
            }
        }
        if (!(mc.currentScreen instanceof GuiChat)) {
            Minecraft.getMinecraft().sfui16.drawStringWithShadow(fpsandping, 3, sr.getScaledHeight() - 17, -1);
            Minecraft.getMinecraft().sfui16.drawStringWithShadow(cords, 3, sr.getScaledHeight() - 8, -1);
            Minecraft.getMinecraft().sfui16.drawStringWithShadow(time, sr.getScaledWidth() - Minecraft.getMinecraft().sfui16.getStringWidth(time) - 18, sr.getScaledHeight() - 17.5,  -1);
            Minecraft.getMinecraft().sfui16.drawStringWithShadow(date, sr.getScaledWidth() - Minecraft.getMinecraft().sfui16.getStringWidth(date) - 7, sr.getScaledHeight() - 8.5,  -1);
        }
    }
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            float f = zLevel;
            zLevel = -90.0F;
            DrawHelper.drawGlowRoundedRect(i - 91, sr.getScaledHeight() - 26, i + 91, sr.getScaledHeight() - 2, new Color(0, 0, 0, 155).getRGB(), 3, 4);
            //DrawHelper.drawSmoothRect(i - 91, sr.getScaledHeight() - 22, i + 91, sr.getScaledHeight(), new Color(0, 0, 0, 165).getRGB());
            if (this.mc.player.inventory.currentItem == 0) {
                DrawHelper.drawSmoothRectBetter(i - 87 + this.mc.player.inventory.currentItem * 20, sr.getScaledHeight() - 6, 16, 2, new Color(92, 113, 206, 255).getRGB());
            } else {
                DrawHelper.drawSmoothRectBetter(i - 87 + this.mc.player.inventory.currentItem * 20, sr.getScaledHeight() - 6, 16, 2, new Color(92, 113, 206, 255).getRGB());
            }
            ...
            for (int l = 0; l < 9; l++) {
                int i1 = i - 90 + l * 20 + 2;
                int j1 = sr.getScaledHeight() - 16 - 7;
            ...
            if (!itemstack.func_190926_b()) {
                ...
                int l1 = sr.getScaledHeight() - 16 - 7;
                if (enumhandside == EnumHandSide.LEFT) {
