...
public abstract class GuiSlot
{
    ...
    public void drawPabloBackground() {
        int red = HClient.getClientColorBackground().getRed();
        int green = HClient.getClientColorBackground().getGreen();
        int blue = HClient.getClientColorBackground().getBlue();
        int alpha = HClient.getClientColorBackground().getAlpha();
    ...
    public void drawScreen(int mouseXIn, int mouseYIn, float partialTicks)
    {
        if (this.visible)
        {
            ...
            if (j1 > 0)
            {
                ...
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(i, this.bottom, 0.0D).tex(0.0D, 1.0D).color(red, green, blue, alpha).endVertex();
                bufferbuilder.pos(j, this.bottom, 0.0D).tex(1.0D, 1.0D).color(red, green, blue, alpha).endVertex();
                bufferbuilder.pos(j, this.top, 0.0D).tex(1.0D, 0.0D).color(red, green, blue, alpha).endVertex();
                bufferbuilder.pos(i, this.top, 0.0D).tex(0.0D, 0.0D).color(red, green, blue, alpha).endVertex();
                tessellator.draw();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
    ...
    protected void overlayBackground(int startY, int endY, int startAlpha, int endAlpha) {
        ...
        GlStateManager.disableTexture2D();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        bufferbuilder.pos(this.left, endY, 0.0D).tex(0.0D, ((float)endY / 32.0F)).color(red, green, blue, alpha).endVertex();
        bufferbuilder.pos((this.left + this.width), endY, 0.0D).tex((float)this.width / 32.0F, (float)endY / 32.0F).color(red, green, blue, alpha).endVertex();
        bufferbuilder.pos((this.left + this.width), startY, 0.0D).tex((float)this.width / 32.0F, (float)startY / 32.0F).color(red, green, blue, alpha).endVertex();
        bufferbuilder.pos(this.left, startY, 0.0D).tex(0.0D, (float)startY / 32.0F).color(red, green, blue, alpha).endVertex();
        tessellator.draw();
        GlStateManager.enableTexture2D();
    }
