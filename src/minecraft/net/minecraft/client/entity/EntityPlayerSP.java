...
import neko.itskekoffcode.clientapi.event.event.EventMessage;
import neko.itskekoffcode.clientapi.event.event.EventMove;
import neko.itskekoffcode.clientapi.event.event.EventPostMotionUpdate;
import neko.itskekoffcode.clientapi.event.event.EventUpdate;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void sendChatMessage(String message) throws Exception {
        ...
        if (CommandManager.executeBot(message)) {
            return;
        }
        if (message.startsWith(CommandManager.syntax) && !message.contains("bots")) {
            Command.msg("&7Type &d" + CommandManager.syntax + "help &7to list all commands", true);
            return;
        }
        if (message.equalsIgnoreCase(CommandManager.botsyntax)) {
            Command.msg("&7Type &d" + CommandManager.syntax + "help bots &7to list all bots commands", true);
            return;
        }
        if (message.startsWith(CommandManager.botsyntax)) {
            Command.msg("&7Type &d" + CommandManager.syntax + "help bots &7to list all bots commands", true);
            return;
        }
