...
public class BotNetwork extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    public BotNetwork(EnumPacketDirection packetDirection) {
        this.direction = packetDirection;
    }

    /**
     * Create a new NetworkManager from the server host and connection it to the server
     */
    public static BotNetwork createNetworkManagerAndConnect(InetAddress address, int serverPort, ProxyParser.Proxy proxy) {
        if (!BotsStopConnectCommand.connecting){ return null; }
        BotNetwork netManager = new BotNetwork(EnumPacketDirection.CLIENTBOUND);
        new Bootstrap().group(CLIENT_NIO_EVENTLOOP.getValue()).handler(new ChannelInitializer() {
                    protected void initChannel(Channel ctx) {
                        ...
                        if (proxy != null) {
                            if (proxy.getType() == ProxyParser.ProxyType.SOCKS4) {
                                final Socks4ProxyHandler s = new Socks4ProxyHandler(proxy.getAddress());
                                s.setConnectTimeoutMillis(9500L);
                                ctx.pipeline().addLast(s);
                            }
                            if (proxy.getType() == ProxyParser.ProxyType.SOCKS5) {
                                final Socks5ProxyHandler s = new Socks5ProxyHandler(proxy.getAddress());
                                s.setConnectTimeoutMillis(9500L);
                                ctx.pipeline().addLast(s);
                            }
                        }
                        ...
                        if (ctx instanceof SocketChannel && ViaMCP.getInstance().getVersion() != ViaMCP.PROTOCOL_VERSION)
                        {
                            UserConnection user = new UserConnectionImpl(ctx, true);
                            new ProtocolPipelineImpl(user);
                            ctx.pipeline().addBefore("encoder", CommonTransformer.HANDLER_ENCODER_NAME, new MCPEncodeHandler(user)).
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        } catch (Throwable ignored) { /*System.out.println("1 + " + ignored.getMessage());*/}
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Packet<?> packet) throws Exception {
        ...
        if (this.channel.isOpen()) {
            ...
            } catch (ThreadQuickExitException ignored) {/* System.out.println("3 + " + ignored.getMessage());*/}
        }
    }
    ...
    public final void sendPacket(Packet<?> packet, GenericFutureListener<? extends Future<? super Void>> listener, GenericFutureListener<? extends Future<? super Void>>... listeners) {
        if (this.isChannelOpen()) {
            ...
            try {
                this.outboundPacketsQueue.add(new InboundHandlerTuplePacketListener(packet, ArrayUtils.add(listeners, 0, listener)));
            } finally {
                this.readWriteLock.writeLock().unlock();
            }
        }
    ...
    private void dispatchPacket(final Packet<?> inPacket, @Nullable final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            this.channel.eventLoop().execute(() -> {
                if (enumconnectionstate != enumconnectionstate1) {
                    BotNetwork.this.setConnectionState(enumconnectionstate);
                }

                ChannelFuture channelfuture1 = BotNetwork.this.channel.writeAndFlush(inPacket);
